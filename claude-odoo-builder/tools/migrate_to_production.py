...
def create_page(client, url, html_file, name, website_id, dry_run):
    """Create a new page on production if it doesn't exist."""
    with open(html_file, "r", encoding="utf-8") as f:
        content = f.read().strip()
    arch = wrap_arch(content, url, name)

    if dry_run:
        log(f"  WOULD CREATE page at {url} ('{name}')", dry_run)
        return

    view_id = client.create("ir.ui.view", {
        "name": name,
        "type": "qweb",
        "key": f"website.page_{url.strip('/').replace('/', '_') or 'home'}",
        "arch": arch,
    })
    ...
    log(f"  CREATED page '{name}' at {url} (view ID {view_id}, page ID {page_id}, unpublished)", dry_run)
...
def main():
    ...
    # Discover production website ID
    sites = client.search_read("website", [], ["id", "name"])
    site_id = sites[0]["id"]
    print(f"Website: '{sites[0]['name']}' (ID {site_id})\n")

    print("1. Homepage arch")
    migrate_page_arch(
        client,
        url="/",
        html_file=os.path.join(TMP, "draft_homepage.html"),
        dry_run=dry_run,
    )
    ...
    if res_pages:
        migrate_page_arch(
            client,
            url="/resources",
            html_file=os.path.join(TMP, "draft_resources.html"),
            dry_run=dry_run,
        )
    else:
        create_page(
